...
sdReadFilePingpong sdStreams[2];
#define PRE_SIZE (1024*1024*128)

void reportFError(FRESULT err){
      LogTextMessage("fatfs err %i",err);
}
...
void BenchmarkBS(int bufsize, int nstreams){
  ...
  for(i=0;i<nstreams;i++){
    ...
    // else LogTextMessage("Open OK...");
    err = f_lseek(&s->f, PRE_SIZE);
    if (err) reportFError(err);
    // else LogTextMessage("lseek1 OK...");
    err = f_lseek(&s->f, 0);
    if (err) reportFError(err);
    // else LogTextMessage("lseek2 OK...");
  ...
  LogTextMessage("BUFSIZE = %i",bufsize);
  LogTextMessage("open : %i ms",t1-t0);
//  LogTextMessage("write : %i ms",t2-t1);
  LogTextMessage("write BW : %i kB/s",(nstreams*nbuf*bufsize)/(t2-t1));
  LogTextMessage("close : %i ms",t3-t2);
  LogTextMessage("open : %i ms",t4-t3);
//  LogTextMessage("read : %i ms",t5-t4,0);
  LogTextMessage("read BW : %i kB/s",(nstreams*nbuf*bufsize)/(t5-t4));
}
